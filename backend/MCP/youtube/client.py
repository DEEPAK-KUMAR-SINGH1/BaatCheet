"""
YouTube OAuth and API helpers for the MCP connector.

The OAuth client secret is expected at backend/MCP/youtube/credentials.json by
default. Override it with YOUTUBE_CLIENT_SECRETS_FILE when needed.
"""
...
def _load_client_config() -> tuple[str, dict[str, Any]]:
    try:
        ...
        raise RuntimeError(
            f"YouTube OAuth client secret at {CLIENT_SECRETS_FILE} is not valid JSON."
        ) from exc
    ...
    if "web" in data:
        ...
        raise RuntimeError(
            "YouTube OAuth client secret must contain a 'web' or 'installed' client."
        )
    ...
    if missing:
        raise RuntimeError(
            "YouTube OAuth client secret is missing required field(s): "
            + ", ".join(missing)
            + "."
        )
    ...
    if client_type == "web" and REDIRECT_URI not in redirect_uris:
        raise RuntimeError(
            "YouTube redirect URI mismatch. Add "
...
def ensure_credentials_file() -> None:
    if not credentials_file_exists():
        raise FileNotFoundError(
            f"YouTube OAuth client secret was not found at {CLIENT_SECRETS_FILE}. "
...
def get_youtube_tools() -> list:
    """Return a list of LangChain tools for YouTube MCP."""
