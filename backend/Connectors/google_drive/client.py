"""
Google Drive OAuth and API helpers for the connector.
...
def _load_client_config() -> tuple[str, dict[str, Any]]:
    data = google_oauth_client_config("GOOGLE_DRIVE", "Google Drive", REDIRECT_URI)
    return "web", data["web"]
...
def get_auth_url(user_email: str) -> str:
    ...
    flow = Flow.from_client_config(
        google_oauth_client_config("GOOGLE_DRIVE", "Google Drive", REDIRECT_URI),
        scopes=SCOPES,
...
def get_google_drive_tools() -> list:
    """Return a list of LangChain tools for Google Drive."""
