"""FastAPI routes for app connections."""
...
from Connectors import get_connector_app, get_connector_connections, iter_connector_apps
...
def _frontend_redirect(app: str, status: str, detail: str = "") -> RedirectResponse:
    params = {"connector": app, "status": status}
    if detail:
...
def _connector_app_or_404(app: str):
    connector_app = get_connector_app(app)
    if not connector_app:
        raise HTTPException(status_code=404, detail=f"Unknown connector app: {app}")
...
def list_connections(current_user=Depends(get_current_user)):
    connections = get_connector_connections(current_user["email"])
    return {
...
def list_apps(current_user=Depends(get_current_user)):
    statuses = {
        item["app"]: item
        for item in get_connector_connections(current_user["email"])
    }
    return {
        "apps": [
            {
...
def app_status(app: str, current_user=Depends(get_current_user)):
    return _connector_app_or_404(app).connection_status(current_user["email"])


@router.post("/{app}/connect")
def app_connect(app: str, current_user=Depends(get_current_user)):
    connector_app = _connector_app_or_404(app)
    client = connector_app.client()
    try:
    ...
    return {
        "app": connector_app.app,
        "name": connector_app.name,
        "auth_url": auth_url,
        "connection": connector_app.connection_status(current_user["email"]),
    }
...
def app_connect_with_token(
    ...
    if not hasattr(client, "save_token"):
        raise HTTPException(status_code=400, detail=f"{connector_app.name} does not support token connection.")

    try:
        if hasattr(client, "exchange_code") and req.extra and req.extra.get("state"):
            client.exchange_code(current_user["email"], req.token, str(req.extra["state"]))
        else:
            client.save_token(current_user["email"], req.token)
    except Exception as exc:
        raise _service_unavailable(exc) from exc
    return connector_app.connection_status(current_user["email"])
