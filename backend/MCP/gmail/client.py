"""
Gmail OAuth and API helpers for the MCP connector.

The OAuth client secret is expected at backend/MCP/gmail/credentials.json by
default. Override it with GMAIL_CLIENT_SECRETS_FILE when needed.
"""
...
def _load_client_config() -> tuple[str, dict[str, Any]]:
    try:
        ...
        raise RuntimeError(
            f"Gmail OAuth client secret at {CLIENT_SECRETS_FILE} is not valid JSON."
        ) from exc
    ...
    if "web" in data:
        ...
        raise RuntimeError(
            "Gmail OAuth client secret must contain a 'web' or 'installed' client."
        )
    ...
    if missing:
        raise RuntimeError(
            "Gmail OAuth client secret is missing required field(s): "
            + ", ".join(missing)
            + "."
        )
    ...
    if client_type == "web" and REDIRECT_URI not in redirect_uris:
        raise RuntimeError(
            "Gmail redirect URI mismatch. Add "
            f"{REDIRECT_URI} to the OAuth client's Authorized redirect URIs "
            "in Google Cloud, then download the updated JSON."
        )
...
def ensure_credentials_file() -> None:
    if not credentials_file_exists():
        raise FileNotFoundError(
            f"Gmail OAuth client secret was not found at {CLIENT_SECRETS_FILE}. "
            "Create a Google OAuth desktop/web client and place the JSON file there, "
